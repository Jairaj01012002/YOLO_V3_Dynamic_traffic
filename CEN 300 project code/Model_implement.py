from tracker import centriod_marker
from tracker import object_tracking
...
def countVehicles(param):
	...
	ct = centriod_marker(maxDisappeared=5, maxDistance=50) # Look into 'CentroidTracker' for further info about parameters
	...
	all_classes = ["person", "bicycle", "car", "motorbike", "bus", "truck"]

	# Classes of interest (with their corresponding indexes for easier looping)
	classes = { 1 : 'bicycle', 2 : 'car', 3 : 'motorbike', 4 : 'bus', 5 : 'truck' }
		...
		while(cap.isOpened()):
			...
			for (objectID, centroid) in objects.items():
				...
				if to is None:
					...
					to = object_tracking(objectID, centroid)
				else:
